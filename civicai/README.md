...
Processes multilingual citizen call transcripts, returns department-specific guidance, and auto-escalates to government workflow when urgent/low-confidence.
...
- `POST /call-assistance` (multilingual call transcript intake + optional escalation)
- `GET /status/{ticket_id}`
- `GET /status` (frontend alias route that opens tracker workspace)
- `GET /history/{user_id}`
- `POST /feedback`
- `GET /analytics`
- `GET /topics`
...
- If Vite shows `Unexpected "export"`, `Identifier has already been declared`, or duplicate export errors, you likely have merge-conflict leftovers or duplicate component/page files. Keep only one default export per file and use these canonical paths: `src/components/Navbar.jsx`, `src/pages/LandingPage.jsx`, `src/pages/AIChatPage.jsx`, `src/pages/ComplaintHistoryPage.jsx`, `src/pages/StatusTrackerPage.jsx`, `src/pages/AdminDashboardPage.jsx`, `src/pages/AnalyticsPage.jsx`. Compatibility shims are also provided for legacy names (`NavBar.jsx`, `Landing.jsx`, `Chat.jsx`, etc.).
- If page loads as a blank white screen, open browser DevTools (`F12`) and check Console. The app now shows an in-page crash panel with the first runtime error. Common causes: stale `node_modules`, mixed branches, or unresolved merge markers. Fix by running `git pull`, deleting `frontend/node_modules`, then `npm install` and `npm run dev`.
- If `npm run dev` fails with `EJSONPARSE` (example: `Expected ',' or '}'` in `package.json`), your local `frontend/package.json` is malformed (usually from manual edits or conflict markers). Restore and reinstall:

```powershell
cd frontend
git checkout -- package.json
# or copy the latest package.json from this branch
npm install
npm run dev
```
...
## Desktop UI notes
- Desktop-first glass workspace with left sidebar + top nav bar.
- Keyboard shortcut: `Ctrl/Cmd + K` focuses chat input in AI workspace.
- Latest generated ticket ID is cached locally and auto-prefilled in tracker.
