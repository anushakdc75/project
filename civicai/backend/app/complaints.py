...
from .nlp.translate import to_english
...
def intake_complaint(
    ...
    try:
        ...
        for attachment in attachments or []:
            ...
            if not content:
                continue
            safe_name = attachment.filename or "attachment.bin"
...
@router.post("/call-assistance", response_model=CallAssistanceResponse)
def call_assistance(payload: CallAssistanceRequest, db: Session = Depends(get_db)):
    try:
        user = _ensure_user_exists(db, payload.user_id, username=payload.citizen_name)
        ai_state = get_ai_state()
        civic_text = to_english(payload.transcript) if payload.language != "en" else payload.transcript
        ai_result = ai_state.run_inference(civic_text)

        should_escalate = (
            "not solved" in civic_text.lower()
            or "urgent" in civic_text.lower()
            or ai_result["confidence"] < 0.45
        )

        ticket_id = None
        status = "guided"
        escalation_note = "Resolved with guidance from call assistant."
        ...
        if should_escalate:
            ...
            complaint = Complaint(
                ticket_id=ticket_id,
                user_id=user.id,
                text=f"CALL ({payload.language}): {payload.transcript}",
            ...
            db.commit()
            status = "escalated"
            escalation_note = "Escalated to government workflow from multilingual call assistance."
            _notify_authority(
                {
                    "ticket_id": ticket_id,
                    "citizen_name": payload.citizen_name,
                    "phone_number": payload.phone_number,
                    "problem": payload.transcript,
                    "department": ai_result["department"],
                    "location": payload.location,
                    "channel": "call_assistance",
                }
            )

        return CallAssistanceResponse(
            ticket_id=ticket_id,
            solution=ai_result["answer"],
        ...
        db.rollback()
        logger.exception("Call assistance failed")
        raise HTTPException(status_code=500, detail=f"Failed to process call assistance: {exc}") from exc
